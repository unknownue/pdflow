...
int main(int argc, char* argv[]){
  ...
  char filename[2048];
  sprintf(filename, "eval/uniformity/pc_point2mesh_distance.txt");
  ...
  std::vector<float> precentage={0.004, 0.006, 0.008, 0.010,0.012};

  std::vector<float> radius(precentage.size());
  sprintf(filename, "eval/uniformity/pc_radius.txt");
  ...
  sprintf(filename, "eval/uniformity/pc_disk_idx.txt");
